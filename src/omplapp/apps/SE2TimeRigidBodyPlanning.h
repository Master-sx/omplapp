...
#ifndef OMPLAPP_SE2_TIME_RIGID_BODY_PLANNING_
#define OMPLAPP_SE2_TIME_RIGID_BODY_PLANNING_
...
namespace ompl
{
    namespace app
    {
        ...
        class SE2TimeRigidBodyPlanning : public AppBase<GEOMETRIC>
        {
        public:

            SE2TimeRigidBodyPlanning(void) : AppBase<GEOMETRIC>(base::StateSpacePtr(new base::SE2TimeStateSpace()), Motion_2D)
            {
                name_ = "Rigid body planning (2D)";
            }

            virtual ~SE2TimeRigidBodyPlanning(void)
            {
            }
